  ...
  if (!jogo) {
    ...
    downloadRoot.innerHTML = `
      <div class="download-card">
        ...
        ${jogo.link
          ? `<a href="${jogo.link}" target="_blank" rel="noopener" class="btn-entrar">⬇ Baixar agora</a>`
...
function atualizarEstadoAudio() {
  if (audioState) audioState.textContent = tocando ? 'ON' : 'OFF';
  if (playBtn) playBtn.textContent = tocando ? '❚❚' : '▶';
  if (muteBtn) muteBtn.textContent = mudo ? '🔇' : '🔊';
}
...
if (muteBtn) {
  muteBtn.addEventListener('click', () => {
    mudo = !mudo;
    if (volumeSlider) volumeSlider.value = mudo ? 0 : (volumeSlider.dataset.last || 50);
    atualizarEstadoAudio();
  });
}
...
if (volumeSlider) {
  volumeSlider.addEventListener('input', () => {
    ...
    tocando = Number(volumeSlider.value) > 0;
    mudo = Number(volumeSlider.value) === 0;
    atualizarEstadoAudio();
  });
}
